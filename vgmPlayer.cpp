...
void Play() {
  ...
  while (1) {
    ...
    switch (data) {
      ...
      // 0xA0 dd : AY8910 Escribe valor dd al registro aa
      case 0xA0:
        ++parseLocation;
        addr = _read_vgm_data(_vgm_song + parseLocation++);
        data = _read_vgm_data(_vgm_song + parseLocation++);
        YMWriteData (addr, data);
        //Serial.print("(addr, data): " + String (addr) + ", " + String (data) + "\n");
        break;
      ...
      case 0x66:
      {
        ...
        if (parseLocation < (0x34 + vgm_data_offset)) 
          parseLocation = 0x34 + vgm_data_offset;
        Serial.println("Loop to addr: " + String (parseLocation));
